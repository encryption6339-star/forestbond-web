...
type CardState = { id: string; category: CategoryId; query: string };
...
function CategoryCard(props: {
  ...
  const { category, query, onQueryChange, onDuplicate, onClose, getByCategory, highlightedIds } = props;
            ...
            return (
              <div
                ...
                onClick={async () => {
                  ...
                  if (await copyText(text)) { setCopiedId(msg.id); setTimeout(() => setCopiedId(null), 1200); }
                }}
              >
...
function layoutToCards(layout: GridLayoutItem[]): CardState[] {
  return layout
    .map((item) => {
      ...
      return { id: String(item.id), category, query: "" };
    })
...
export function MapPageClient() {
  ...
  const mountPointsRef = useRef<Map<string, HTMLDivElement>>(new Map());
  ...
  const createWidget = useCallback((card: CardState, layoutItem?: Partial<GridLayoutItem>) => {
    if (!gridRef.current || !gridElRef.current) return;
    const widget = document.createElement("div");
    widget.className = "grid-stack-item";
    widget.setAttribute("gs-id", card.id);
    const content = document.createElement("div");
    content.className = "grid-stack-item-content";
    const host = document.createElement("div");
    host.className = "map-card-host";
    host.style.height = "100%";
    content.appendChild(host);
    widget.appendChild(content);
    mountPointsRef.current.set(card.id, host);
    gridElRef.current.appendChild(widget);
    gridRef.current.makeWidget(widget, { id: card.id, w: layoutItem?.w ?? 4, h: layoutItem?.h ?? 4, x: layoutItem?.x, y: layoutItem?.y });
  }, []);

  const addCard = useCallback((category: CategoryId, layoutItem?: Partial<GridLayoutItem>) => {
    const card: CardState = { id: layoutItem?.id ?? \`\${category}-\${Date.now()}\`, category, query: "" };
    setCards((prev) => [...prev, card]);
    requestAnimationFrame(() => { createWidget(card, layoutItem); saveLayout(); });
  }, [createWidget, saveLayout]);

  const removeCard = useCallback((id: string) => {
    const node = gridElRef.current?.querySelector(\`.grid-stack-item[gs-id="\${id}"]\`);
    if (node && gridRef.current) gridRef.current.removeWidget(node as HTMLElement, true);
    mountPointsRef.current.delete(id);
    setCards((prev) => prev.filter((c) => c.id !== id));
    saveLayout();
  }, [saveLayout]);

  const resetLayout = useCallback(() => {
    if (!gridRef.current || !confirm("레이아웃을 초기화할까요?")) return;
    localStorage.removeItem(LAYOUT_STORAGE_KEY);
    gridRef.current.removeAll(true);
    mountPointsRef.current.clear();
    const layout = DEFAULT_LAYOUT.map((item) => ({ ...item }));
    const next = layoutToCards(layout);
    setCards(next);
    requestAnimationFrame(() => {
      next.forEach((card, i) => createWidget(card, layout[i]));
      saveLayout();
    });
  }, [createWidget, saveLayout]);
  ...
  useEffect(() => {
    ...
    const grid = GridStack.init({ column: 12, cellHeight: 70, margin: 8, float: true, animate: true, draggable: { handle: ".card-header" }, resizable: { handles: "se" } }, gridElRef.current);
    gridRef.current = grid;
    const layout = loadLayout();
    layoutToCards(layout).forEach((card, i) => createWidget(card, layout[i]));
    const onChange = () => saveLayout();
    grid.on("change", onChange);
    return () => { grid.off("change"); grid.destroy(true); gridRef.current = null; mountPointsRef.current.clear(); };
  }, [createWidget, saveLayout]);
  ...
  useEffect(() => {
    return subscribe((event, data) => {
      if (event === "update" && Array.isArray(data)) {
        ...
        setFeedTick((n) => n + 1);
        setTimeout(() => { highlightedRef.current.clear(); setFeedTick((n) => n + 1); }, 3000);
      }
      if (event === "snapshot" || event === "update") setFeedTick((n) => n + 1);
    });
  }, [subscribe]);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><ConnectionBadge status={status} /><button type="button" className="btn" onClick={resetLayout}>레이아웃 초기화</button></>}
        below={<>
          ...
          {tipsOpen ? <div className="tips-panel"><ul>
      ...
      {cards.map((card) => {
        const host = mountPointsRef.current.get(card.id);
        if (!host) return null;
        return createPortal(
          <CategoryCard
            key={\`\${card.id}-\${feedTick}\`}
            category={card.category}
            query={card.query}
            onQueryChange={(value) => setCards((prev) => prev.map((c) => (c.id === card.id ? { ...c, query: value } : c)))}
            onDuplicate={() => addCard(card.category, { w: 4, h: 4 })}
      ...
      <p className="footer-note">포털사이트를 통해 제공되는 정보는 단순 참고용이며 오류·지연이 발생할 수 있습니다. 정보 이용에 따른 책임은 이용자 본인에게 있습니다.</p>
    </>
  );
}
`;
fs.writeFileSync("C:/forestbond-clone/web/src/components/MapPageClient.tsx", content, "utf8");
console.log("written");
